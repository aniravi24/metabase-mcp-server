...
export function addDashboardTools(server, metabaseClient) {
    ...
    server.addTool({
        name: "add_text_block",
        description: "Add a text block or heading to a dashboard - use this for explanatory text, titles, or instructions",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
        ...
        execute: async (args) => {
            try {
                ...
                const result = await metabaseClient.addCardToDashboard(args.dashboard_id, cardData);
                return JSON.stringify(result, null, 2);
            }
            catch (error) {
                throw new Error(`Failed to add text block to dashboard ${args.dashboard_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
    /**
    ...
    server.addTool({
        ...
        description: "⚠️ DANGER: REPLACES ALL dashboard cards - any cards not in the array will be DELETED. To update a single card, first get_dashboard to fetch ALL cards, then include ALL of them with your modifications. This affects ALL tabs.",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
            cards: z
        ...
        execute: async (args) => {
            try {
                const result = await metabaseClient.updateDashboardCards(args.dashboard_id, args.cards, args.tabs);
                return JSON.stringify(result, null, 2);
            }
            catch (error) {
    ...
    server.addTool({
        ...
        description: "Remove specific dashcards from a dashboard by their dashcard IDs (not card_id) - use this to clean up dashboards or reorganize content",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
        ...
        execute: async (args) => {
            try {
                const result = await metabaseClient.removeCardsFromDashboard(args.dashboard_id, args.dashcard_ids);
                return JSON.stringify(result, null, 2);
            }
            catch (error) {
    ...
    server.addTool({
        ...
        description: "Update a specific dashcard's properties without affecting other cards - use for parameter_mappings, visualization_settings, position, or size changes. Much safer than update_dashboard_cards.",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
        ...
        execute: async (args) => {
            try {
                const result = await metabaseClient.updateDashcard(args.dashboard_id, args.dashcard_id, args.updates);
                return JSON.stringify(result, null, 2);
            }
            catch (error) {
                throw new Error(`Failed to update dashcard ${args.dashcard_id} on dashboard ${args.dashboard_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
    /**
    ...
    server.addTool({
        ...
        description: "Extract all queries from a dashboard with IDs resolved to actual table/column names - use this to understand dashboard data sources, audit queries, or plan migrations",
        metadata: { isRead: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
        }).strict(),
        execute: async (args) => {
            try {
                ...
                // Build tab lookup
                const tabLookup = {};
                tabs.forEach((t) => { tabLookup[t.id] = t.name; });
                ...
                const resolveFieldRef = (ref) => {
                    ...
                    return ref.map((item) => resolveFieldRef(item));
                };
                // Helper to resolve entire MBQL query
                const resolveMbql = (query) => {
                    if (!query)
                        return query;
                    const resolved = {};
                    ...
                    if (query.aggregation) {
                        resolved.aggregation = query.aggregation.map((agg) => resolveFieldRef(agg));
                    }
                    // Resolve breakout
                    if (query.breakout) {
                        resolved.breakout = query.breakout.map((b) => resolveFieldRef(b));
                    }
                    // Resolve filter
                    if (query.filter) {
                    ...
                    // Resolve order-by
                    if (query['order-by']) {
                        resolved['order-by'] = query['order-by'].map((o) => resolveFieldRef(o));
                    }
                    // Resolve joins
                    if (query.joins) {
                        resolved.joins = query.joins.map((j) => ({
                            ...j,
                    ...
                    if (query.fields) {
                        resolved.fields = query.fields.map((f) => resolveFieldRef(f));
                    }
                ...
                throw new Error(`Failed to get queries for dashboard ${args.dashboard_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
    /**
    ...
    server.addTool({
        ...
        description: "Analyze dashboard filter connections to find unconnected or misconfigured cards - use this to diagnose filter issues and ensure all cards are properly connected",
        metadata: { isRead: true },
        parameters: z.object({
        ...
        execute: async (args) => {
            try {
                ...
                const cardAudit = dashcards.map((dc) => {
                    const card = dc.card || {};
                    const mappings = dc.parameter_mappings || [];
                    const connectedParams = mappings.map((m) => m.parameter_id);
                    const missingParams = parameterIds.filter((pid) => !connectedParams.includes(pid));
                    ...
                    mappings.forEach((m) => {
                        if (!m.target) {
                            ...
                            const hasStageNumber = m.target.some((t) => typeof t === 'object' && t !== null && 'stage-number' in t);
                            if (!hasStageNumber) {
                ...
                const cardsWithIssues = cardAudit.filter((c) => c.missing_params.length > 0 || c.errors.length > 0);
                return JSON.stringify({
                    dashboard_id: args.dashboard_id,
                ...
                throw new Error(`Failed to audit dashboard ${args.dashboard_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
}
