...
export function addTableTools(server, metabaseClient) {
    ...
    server.addTool({
        ...
        execute: async (args) => {
            try {
                const result = await metabaseClient.getFieldByName(args.table_id, args.column_name);
                return JSON.stringify(result, null, 2);
            }
            catch (error) {
                throw new Error(`Failed to find field '${args.column_name}' in table ${args.table_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
}
